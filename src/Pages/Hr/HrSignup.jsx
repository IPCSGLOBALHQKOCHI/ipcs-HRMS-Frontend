...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("http://localhost:5000/api/hr/hrsignup", formData);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-md w-96">
        <h2 className="text-2xl font-semibold text-center mb-4">HR Signup</h2>
        ...
        <p className="text-center mt-3 text-sm">Already have an account? <a href="/hrlogin" className="text-blue-500">Login</a></p>
      </div>
    </div>
  );
};
